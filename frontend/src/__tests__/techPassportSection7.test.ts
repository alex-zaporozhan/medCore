...
/**
 * Регрессия инвариантов `ARCH_FRONTEND_TECH_PASSPORT_DENTAL_BOOKING.md` §7
 * (план реализации — фаза 7): префикс `/api` и ключи хранилища, зоны `/admin`/`/app` и guard'ы,
 * отсутствие параллельного UI-kit; см. также ESLint `no-restricted-imports`.
 */
describe("TECH_PASSPORT §7 (forbidden without epic)", () => {
  ...
  it("vite dev proxy keeps /api prefix (align with §3.1 / deploy)", () => {
