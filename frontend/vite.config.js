...
function originHealthOk(origin) {
    return new Promise((resolve) => {
...
async function resolveApiProxyTarget() {
    ...
    for (const candidate of API_PROXY_CANDIDATES) {
        if (await originHealthOk(candidate.origin))
            return candidate.origin;
    }
    for (const candidate of API_PROXY_CANDIDATES) {
        if (await portOpen(candidate.port))
            return candidate.origin;
    }
    return "http://127.0.0.1:8010";
}
async function refreshLiveProxyTarget() {
...
function liveApiProxyPlugin() {
    let pollStarted = false;
    const startPoll = () => {
        if (pollStarted)
            return;
        if ((process.env.VITE_API_PROXY_TARGET || "").trim())
            return;
        pollStarted = true;
        ...
        plugins: [
            ...
            VitePWA({
                ...
                manifest: {
                    ...
                    short_name: "ЕСУ",
                    description: "Запись, чат с организацией, профиль и уведомления — PWA для клиентов сервисных компаний.",
                    lang: "ru",
                    start_url: "/app",
                    scope: "/",
                    display: "standalone",
