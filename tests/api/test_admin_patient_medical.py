...
async def test_admin_medical_visits_crud_minimal(client, admin_auth, seed_data):
    ...
    # RBAC depends on seeded roles; accept 200 in baseline environments
    assert r0.status_code in (200, 403), r0.text
    if r0.status_code != 200:
        pytest.skip("RBAC denies patients.medical.read in this environment")
...
async def test_admin_medical_file_download_token_and_stream_contract(client, admin_auth, seed_data):
    ...
    assert tok.status_code in (200, 403), tok.text
