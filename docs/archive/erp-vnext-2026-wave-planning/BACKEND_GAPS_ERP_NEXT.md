...
- **Наблюдаемость L2 / цепочка Booking→ERP:** пороги алертов и кардинальность метрик зафиксированы в **`./NONFUNCTIONAL_AUDIT_NEXT.md`** (§6); правила Prometheus — **`deploy/prometheus/dental_booking_alerts.yml`**. Метрики цепочки completion используют **`clinic_bucket`** (32 корзины), nightly failures — **`aggregate_kind`** без `clinic_id` в лейблах.
- **Миграции watermark / event refresh:** **`../../MIGRATION_UPGRADE.md`** (блок ERP витрины).
