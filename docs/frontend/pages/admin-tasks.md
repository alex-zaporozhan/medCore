...
- URL **`/admin/tasks`** (в примере query **`?stream=<uuid>`** — выбранный поток). Заголовок **«Задачи»**, кнопки **«Новая задача»**, фильтры (поиск, исполнитель, сроки, **«Фильтры»**).
- Верхние зоны: **«Все потоки»**, **«Требуют подтверждения»** (очередь пуста / счётчик), блок **«Список задач»** / **«Назначено мне»**.
- Колонки Kanban (пример): **Открыто**, **В работе**, **На паузе**, **На проверке** — у колонок метрики **лимит**, **SLA просрочено**, **в работе 48ч+**; на карточках — статус, приоритет, срок, инициалы исполнителя.
- Модалка **«Новая задача»**: **Заголовок задачи**, **Описание**, **Исполнители**, **Приоритет**, **Поток**, **Теги** (+ тег), **Срок** (дата + время).
- Модалка деталей задачи (пример **«Обзвон»**): блоки исполнителей, потока/тегов, срока и статуса, кнопки перевода (**«В работу»**, **«На проверку»**, **«Завершить»**), блокировка, лента **«Комментарии»**.
...
- **gap:** отдельных тестов страницы не найдено; риск регрессий при правках DnD и модалок.
