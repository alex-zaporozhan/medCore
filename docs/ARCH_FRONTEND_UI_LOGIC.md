...
**Назначение:** единая точка для **внедрения** норм светлой рабочей зоны и токенов **Swiss Slate / Ink** в этом репозитории. Общий канон UI-логики — `docs/TECH_PASSPORT_FRONTEND_UI_LOGIC.md` (§1–6, **§7** светлая тема, **§8** Roadmap, **§9** Premium Micro-Design Codex — **эталон микро-вёрстки Mantine v7**). Продуктовый концепт палитры и эффектов — `docs/artifacts/85 plus/DESIGN_ENTERPRISE_85_PLUS_CONCEPT.md` **§3.6**. Маршруты и стек — `docs/artifacts/ARCH_FRONTEND_TECH_PASSPORT_DENTAL_BOOKING.md`.
...
В `createTheme` заданы **многослойные** тени с холодным подтоном (не чистый `#000`), см. `DESIGN_ENTERPRISE_85_PLUS_CONCEPT.md` §3.6.4:
...
- `docs/artifacts/DEV_PROMPTS_ADMIN_CRISP_SAAS_UI_2026.md` — **промпты для @DEV**: порядок работ, DoD, единый блок для внедрения §7 глобально.

- `docs/artifacts/ARCH_FRONTEND_DESIGN_SYSTEM_MIDNIGHT.md` — историческая/текущая визуальная база; при конфликте имен — решение через эпик и обновление обоих документов.

- `docs/artifacts/85 plus/LEAD_DESIGN_UNIFICATION_ROUTE_SWISS_SLATE_INK.md` — маршрут унификации палитры для @LEAD.

- `docs/TEMPLATE_DESIGN_UX.md` — только маркетинговые страницы, не админка.



Version: 1.3 | 2026-03-27 — Swiss Slate / Ink (`brand`), тени §3.6.4, синхронизация с `DESIGN_ENTERPRISE_85_PLUS_CONCEPT.md`
