...
- **Время:** все метки времени в UTC; политика и границы с внешними API — см. [ARCH_TIMEZONE.md](ARCH_TIMEZONE.md)
...
В этом проекте Postgres и Redis в Docker проброшены на **5433** и **6380** (чтобы не конфликтовать с локальными 5432/6379). В `.env` для запуска с хоста (uvicorn, alembic, pytest) укажите:
- `DATABASE_URL=...@localhost:5433/dental_booking`
- `REDIS_URL=redis://localhost:6380/0` (и при необходимости `REDIS_URL_TEST=redis://localhost:6380/0`, `DATABASE_URL_TEST=...@localhost:5433/dental_booking_test`).
...
1. **Порты 5432 или 6379 заняты** — другой процесс (локальный Postgres/Redis, другой контейнер) держит порт. Проверка (PowerShell):
   ...
   ```
   Если порт занят: остановите процесс или смените порты в `docker-compose.yml` (например `"5433:5432"` для postgres) и в `.env` (если подключаетесь с хоста — `localhost:5433`).
...
Один раз создайте БД для тестов (команда идемпотентна — не упадёт, если БД уже есть):

```powershell
# PowerShell (одинарные кавычки, внутри строки '' для одной кавычки):
docker exec dental_booking_postgres psql -U postgres -c 'DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_database WHERE datname = ''dental_booking_test'') THEN CREATE DATABASE dental_booking_test; END IF; END $$;'
# Либо просто (если БД уже есть — будет ошибка "already exists", это нормально):
docker exec dental_booking_postgres psql -U postgres -c "CREATE DATABASE dental_booking_test;"
```

В `.env` должны быть заданы `DATABASE_URL_TEST` и `REDIS_URL_TEST` (см. .env.example). Затем: `poetry run pytest tests/ -v`.
