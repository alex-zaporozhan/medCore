...
- **Lag / observability:** `GET /health/replica` probes the reporting DSN and returns `lag_seconds` on standbys; gauge `db_replica_lag_observed_seconds` updates on each probe. Runbook: `docs/artifacts/WAVE5_OPS_RUNBOOK.md`.  
- **Index audit:** example `EXPLAIN` queries after migration `w5perf1idx_fin` — `docs/artifacts/WAVE5_A3_EXPLAIN_QUERIES.sql`.
