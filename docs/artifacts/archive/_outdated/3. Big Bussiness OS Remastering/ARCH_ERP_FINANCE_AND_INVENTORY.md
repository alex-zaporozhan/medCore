...
### 3.2. Варианты источников суммы

Phase 1 (минимально жизнеспособный вариант):

- выручка = сумма цен услуг, привязанных к `Booking`;
- если есть `Payment`:
  - можно разделять «предоплата» и «доплата», но для ERP важно, что **полная стоимость услуги отражена как приход** (деньги могут поступить частями, но в рамках MVP допустимо учитывать по факту выполнения визита).
