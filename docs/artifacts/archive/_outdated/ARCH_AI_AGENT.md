...
### 4.4. Аудит действий агента (реализация V2)

- **Хранение ToolEvent:**
  - Добавлена сущность `AiToolEvent` (`ai_tool_events`) с полями:
    - `clinic_id`, `chat_id`, `message_id`;
    - `tool_name`, `status` (`success|error`), `error_code`;
    - `args`, `result`, `note`, `created_at`.
  - Каждый вызов инструмента из `run_ai_agent` порождает запись:
    - при `ToolError` → `status="error"`, `error_code=result.code`, `args = args.model_dump()`, без `result`;
    - при успехе → `status="success"`, `result=result.model_dump()`.
  - В `AgentResult.tool_events` дублируется агрегированная информация по вызванным инструментам.
- **Сервисные сообщения в Omnichannel:**
  - После отправки основного AI‑ответа в чат создаётся системное исходящее сообщение:
    - `actor_type="SYSTEM"`, `content`:
      - `AI_ACTION_SUCCESS: tools=...` — если есть успешные инструменты;
      - либо `AI_ACTION_ERROR: no successful tool actions` — если ни один инструмент не сработал.
  - Это сообщение отображается в чате для админов и прозрачно фиксирует, что сделал агент.
- **Лимиты цикла и таймауты:**
  - В `run_ai_agent` зафиксированы:
    - `max_tool_iterations=3` — максимальная глубина цикла Function Calling;
    - `max_total_duration_seconds=20` — глобальный таймаут выполнения агента, после которого цикл tool‑вызовов обрывается с безопасным ответом для клиента.
