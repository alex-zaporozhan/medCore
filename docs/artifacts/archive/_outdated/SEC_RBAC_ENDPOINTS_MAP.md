...
> **Инвентарь кодов (CI gate):** `docs/artifacts/sec_rbac_router_permissions.txt` — полный список permission-строк из `src/api/v1/routers/*.py`. После добавления роутов: `python scripts/audit_rbac_endpoints.py --write`, затем `pytest tests/application/test_sec_rbac_router_permissions_inventory.py`.  
