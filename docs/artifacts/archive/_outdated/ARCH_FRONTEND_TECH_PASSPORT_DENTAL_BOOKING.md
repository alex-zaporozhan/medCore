...
> **Назначение:** единая точка правды для маршрутов, зон, стека и слоя данных. Поведение экранов (Drawer, таблицы, сущности) — в `docs/TECH_PASSPORT_FRONTEND_UI_LOGIC.md`. Визуал лендинга — в `docs/TEMPLATE_DESIGN_UX.md`.
...
**Визуальный канон (Midnight & Graphite, SaaS «инструмент»):** `docs/artifacts/ARCH_FRONTEND_DESIGN_SYSTEM_MIDNIGHT.md` — палитра, токены, навигация админки, экономия внимания на плотных панелях (в т.ч. Omni Chat), E2E-устойчивость.
...
**Прод-база и масштаб (фронт + мост):** `docs/artifacts/ARCH_FRONTEND_ENTERPRISE_BASELINE.md`. Отказоустойчивость бэкенда/БД/очередей — отдельные треки (в т.ч. `QA_ARCH_85_PLUS_8W_EXECUTION_TRACKER.md`).
