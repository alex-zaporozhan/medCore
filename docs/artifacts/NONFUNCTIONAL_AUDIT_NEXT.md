...
- **Lag probe:** `GET /health/replica` (если задан `DATABASE_REPLICA_URL`) — `in_recovery`, `lag_seconds`, `lag_warning` при превышении `DB_REPLICA_LAG_WARN_SECONDS`; gauge **`db_replica_lag_observed_seconds`**. Runbook: `docs/artifacts/WAVE5_OPS_RUNBOOK.md`.
...
- **Индексы / EXPLAIN:** после `w5perf1idx_fin` — см. `docs/artifacts/WAVE5_A3_EXPLAIN_QUERIES.sql`.
...
**Остаточная кардинальность:** часть CRM-воронки (`crm_lead_stage_transitions_total` и др.) всё ещё может использовать `clinic_id` в лейблах — уровень **B/C**; см. `docs/artifacts/QA_ARCH_POST_WAVES_FUNDAMENTALS_BACKLOG.md` при плановом сокращении.
...
**Perf «на потом»** (без DoD W4): горячий путь `stage-semantics` при большом числе стадий, объединённый bootstrap Omni — см. **`docs/artifacts/QA_ARCH_POST_WAVES_FUNDAMENTALS_BACKLOG.md`** §5 п.13–15.
...
- `docs/artifacts/QA_ARCH_W1_W2_FOLLOWUP_PLAN_2026.md` — чеклист доработок по отчёту W1/W2 (перекрёстно с этим файлом)
- `docs/artifacts/DEV_PROMPT_QA_ARCH_UNIFIED_BACKLOG.md`, `CONTRIBUTING.md` — волны W1/W2 и критерии PR (§5)
