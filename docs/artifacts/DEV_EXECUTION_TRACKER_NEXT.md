...
> **Файл:** `docs/artifacts/DEV_EXECUTION_TRACKER_NEXT.md`. Связанные артефакты: [`DEV_PROMPTS_NEXT.md`](./DEV_PROMPTS_NEXT.md), [`DEV_WORKFLOW_GUIDE_NEXT.md`](./DEV_WORKFLOW_GUIDE_NEXT.md), [`ARCH_DEV_COVERAGE_NEXT.md`](./ARCH_DEV_COVERAGE_NEXT.md). Имена `ARCH_*_NEXT.md` / `BACKEND_GAPS_*` без пути — в **`docs/artifacts/`**.
...
- **Источник порядка:** использовать зависимости и приоритеты из [`DEV_PROMPTS_NEXT.md`](../PROMPTS/DEV_PROMPTS_NEXT.md) (раздел 3, строки 293–319).  
  Сначала блок 1 (P0), затем 2, 3, …, 8.
- **Контракт по эскалации к `@LEAD`:** когда менять архитектуру/контракты, а когда действовать самостоятельно, описано в [`DEV_WORKFLOW_GUIDE_NEXT.md`](./DEV_WORKFLOW_GUIDE_NEXT.md).
...
- **2026-03-20 (@QA_ARCH / @DEV):** в §0 добавлен контракт **«источник правды закрытия пакета»** (детали сдачи — в парном `docs/artifacts/ARCH_DEV_<ID>_TASKS.md`); в цитате про фазы 1–6 указан полный путь к `docs/artifacts/ARCH_DEV_COVERAGE_NEXT.md`. Митигация выборочного аудита: [`QA_ARCH_AUDIT_ARCH_AND_CODE_PRE_007_50PCT.md`](./QA_ARCH_AUDIT_ARCH_AND_CODE_PRE_007_50PCT.md); изоляция ошибок в `EventBus` и метрика `domain_event_handler_failures_total` — см. `ARCH_AUDIT_NEXT.md` §4.
...
- **2026-03-20 (@DEV):** **Wave 2.2** (**A15**, **A18**): **`ORDER BY`** для materials (raw + витрина) и attribution (raw, согласовано с витриной); описания **`items`** в DTO для OpenAPI; артефакт **`docs/artifacts/ERP_VITRINES_L2_EXPLAIN_ATTRIBUTION.md`** (EXPLAIN/индексы ROI/attribution).
...
- **2026-03-20 (@QA_ARCH):** закрытие рисков отчёта W1/W2 — **`docs/artifacts/NONFUNCTIONAL_AUDIT_NEXT.md`** (§5–§6); **`clinic_bucket`** для цепочки Booking→ERP; **`erp_aggregate_nightly_kind_failures_total`** без `clinic_id`; **`erp_aggregate_empty_trusted_total`**; **`docs/MIGRATION_UPGRADE.md`** (watermark + event env); **`docs/artifacts/QA_ARCH_W1_W2_FOLLOWUP_PLAN_2026.md`**; обновлены **`BACKEND_GAPS_ERP_NEXT.md`**, **`ARCH_DEV_ERP_VITRINES_026_TASKS.md`** (nightly semantics).
- **2026-03-21 (@LEAD / @QA_ARCH):** **`DEV_PROMPT_QA_ARCH_UNIFIED_BACKLOG` Wave 3** — **закрыта**; решения зафиксированы в **`docs/artifacts/LEAD_DECISIONS_QA_ARCH_WAVES.md`** (канон политики Loyalty = **`ARCH_DEV_ERP_LOYALTY_011`**, единый PR для W3 допустим, pytest/head миграций = операционный gate). Обновлены **`DEV_PROMPT_QA_ARCH_UNIFIED_BACKLOG.md`**, **`QA_ARCH_POST_WAVES_FUNDAMENTALS_BACKLOG.md`**, **`ARCHITECTURE_EXCELLENCE_PASSPORT.md`** §16.
