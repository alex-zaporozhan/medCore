...
**Канон визуала:** `DESIGN_ENTERPRISE_85_PLUS_CONCEPT.md` **§3.6** (токены, тени, кнопки, эффекты).  
...
2. **Код** — тема + CSS-токены → shell → P0 страницы (по roadmap из `DESIGN_ENTERPRISE_85_PLUS_CONCEPT.md` §8).
...
| Файл | Зачем |
|------|--------|
| `docs/artifacts/85 plus/DESIGN_ENTERPRISE_85_PLUS_CONCEPT.md` | §3.2–3.6 — роли цвета, токены, elevation, кнопки, эффекты |
...
| P2 | Страницы из инвентаря `DESIGN_ENTERPRISE_85_PLUS_CONCEPT.md` §2 | Убрать ad-hoc hex; P0 экраны первыми |
...
| `docs/artifacts/DEV_PROMPTS_ADMIN_CRISP_SAAS_UI_2026.md` | Заменить чеклисты `indigo` → **ink/brand**; ссылка на `DESIGN_ENTERPRISE_85_PLUS_CONCEPT.md` §3.6 |
| `docs/artifacts/85 plus/DESIGN_TOKENS_85_PLUS.json` | Обновить `color.surface.main` → `#f4f6f8`, `primary.*` → шкала **ink** из §3.6 / свотчей; `meta.source` + версия |

### C2 — Продуктовые / LEAD / дизайн-процесс

| Файл | Что сделать |
|------|----------------|
| `docs/artifacts/85 plus/LEAD_DESIGN_IMPLEMENTATION_PLAYBOOK_85_PLUS.md` | Step 1 Token adoption: явная ссылка на **Swiss Slate / Ink** и `LEAD_DESIGN_UNIFICATION_ROUTE_SWISS_SLATE_INK.md` |
| `docs/artifacts/85 plus/LEAD_DESIGNER_TZ_ENTERPRISE_85_PLUS.md` | Секции про цвет/primary — выровнять с §3.6 |
| `docs/artifacts/85 plus/DEV_A_TO_B_EXECUTION_PATH_85_PLUS.md` | Если есть пункты про «визуал / тема» — одна строка: канон палитры §3.6 |
...
| Файл | Что сделать |
|------|----------------|
| `docs/artifacts/ARCH_FRONTEND_ENTERPRISE_BASELINE.md` | Дополнить примечанием: визуальный канон 85+ см. `DESIGN_ENTERPRISE_85_PLUS_CONCEPT.md` §3.6 (не полагаться только на «indigo» из старых roadmap) |
...
| `docs/artifacts/QA_ARCH_85_PLUS_ROADMAP.md` | Критерий приёмки UI: соответствие токенам Swiss Slate / Ink |
...
- [x] `DESIGN_TOKENS_85_PLUS.json` совпадает с каноном §3.6 (v1.1.0).
...
- [ ] `DESIGN_ENTERPRISE_85_PLUS_CONCEPT.md` §9 DoD закрыт по токенам (после финального sign-off @LEAD).
