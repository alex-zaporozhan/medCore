...
> **Назначение:** единая точка правды для маршрутов, зон, стека и слоя данных. Поведение экранов (Drawer, таблицы, сущности) — в `docs/TECH_PASSPORT_FRONTEND_UI_LOGIC.md`. Светлая тема админки, токены и чеклист внедрения — `docs/ARCH_FRONTEND_UI_LOGIC.md` (канон §7 — `TECH_PASSPORT`). Визуал лендинга — в `docs/TEMPLATE_DESIGN_UX.md`.
...
**Трассируемость (закрытие фаз 0–1, @QA_ARCH):** сверка маршрутов §2 с `App.tsx`, гейты `npm test` / `npm run build` — `docs/artifacts/ARCH_FRONTEND_TECH_PASSPORT_DEV_IMPLEMENTATION_PLAN.md` (блок «Статус закрытия фаз»).

**Тема:** `frontend/src/theme.ts` · **Глобальные токены:** `frontend/src/index.css` (`:root`, классы вроде `.glass-light` — см. `docs/ARCH_FRONTEND_UI_LOGIC.md`).

**Визуальная база (Midnight & Graphite; полный рефреш дизайн-системы запланирован — см. шапку):** `docs/artifacts/ARCH_FRONTEND_DESIGN_SYSTEM_MIDNIGHT.md` — палитра, токены, навигация админки, экономия внимания на плотных панелях (в т.ч. Omni Chat), E2E-устойчивость; до публикации новой версии документа опирайтесь также на код (`theme.ts`, `index.css`). Целевой **Crisp SaaS** для светлой зоны main — `docs/TECH_PASSPORT_FRONTEND_UI_LOGIC.md` §7 и `docs/ARCH_FRONTEND_UI_LOGIC.md`.
...
Канон вёрстки: `docs/TEMPLATE_DESIGN_UX.md`.
...
**Прод-база и масштаб (фронт + мост):** `docs/artifacts/ARCH_FRONTEND_ENTERPRISE_BASELINE.md`. Отказоустойчивость бэкенда/БД/очередей — отдельные треки (в т.ч. `docs/artifacts/85 plus/QA_ARCH_85_PLUS_8W_EXECUTION_TRACKER.md`).

Детальные контракты эндпоинтов — в `docs/artifacts/ARCH_DEV_*.md` и OpenAPI/роутерах бэкенда (`src/api/v1/`).
...
| Тема | Документ |
|------|----------|
| Поведение админки и PWA (Drawer, таблицы, сущности) | `docs/TECH_PASSPORT_FRONTEND_UI_LOGIC.md` |
| **Omni Chat** — сетка, плотность inbox, схлопывание правого инспектора, хоткеи | §2.2.1 этого файла; `docs/artifacts/ARCH_FRONTEND_DESIGN_SYSTEM_MIDNIGHT.md` §5.1 |
| **Оболочка правых панелей админки** (`AdminDrawer`, `shellPanelStyles`, миграция с Mantine `Drawer`; фазы B+ — меню/IA отдельно) | `docs/artifacts/ARCH_FRONTEND_ADMIN_SHELL_DRAWER_2026.md` |
| Чеклисты страниц и приёмка | `docs/DOMAIN_STANDARDS.md` |
| Маркетинг, hero, glass, производительность motion | `docs/TEMPLATE_DESIGN_UX.md` |
| Два визуальных контура, роль @FRONTEND | `docs/ROLE_FRONTEND.md` |
| NFR и зрелость | `docs/ARCHITECTURE_EXCELLENCE_PASSPORT.md` |
...
## 8. Связанные артефакты в `docs/artifacts/`
...
- **Фронт и программа 8.5+ (отложенные эпики, стыковка с неделями):** `ARCH_FRONTEND_85_PLUS_ALIGNMENT.md` (v1.4: §6–§8 перформанс «на потом», в т.ч. доп. таблица §8) · трекер: `docs/artifacts/85 plus/QA_ARCH_85_PLUS_8W_EXECUTION_TRACKER.md` (§8 выжимка и полная программа)
