...
Детали и таблица: **`docs/artifacts/QA_ARCH_POST_WAVES_FUNDAMENTALS_BACKLOG.md`** (§«Выполнено Wave 4»). NFR: **`docs/artifacts/NONFUNCTIONAL_AUDIT_NEXT.md`** §6.6.
...
| **A3** | Частично | миграция **`w5perf1idx_fin`**; шаблоны **EXPLAIN** — `docs/artifacts/WAVE5_A3_EXPLAIN_QUERIES.sql` |
...
Детали OPS: **`docs/artifacts/WAVE5_OPS_RUNBOOK.md`**; NFR: **`docs/artifacts/NONFUNCTIONAL_AUDIT_NEXT.md`** §5.3. (ADR Wave 5 — отдельный файл в `docs/adr/` репозитория.)
...
### Мини-эпик P2 — Clients & Schedule (follow-up QA, 2026-03)

После закрытия объёма **фазы P2** в коде ревью @QA_ARCH выделило хвосты (семантика PATCH `notes`, политика доступа к `GET /patients`, тесты, UX). **Не дублирует** таблицы §A–K ниже — отдельный файл:

- **`QA_ARCH_P2_CLIENTS_SCHEDULE_FOLLOWUP_2026.md`** — ID **P2-FU1…P2-FU7**, ссылка из `ARCH_PHASE_02_CLIENTS_SCHEDULE_2026.md` §9.
...
| SR8 | `docs/SEC_RBAC_SPEC.md` | Вынести матрицу из ARCH. |
| SR9 | Параметризованные тесты матрицы | Snapshot / таблица роль×permission. |

---

## Приложение: охват файлов `docs/artifacts/ARCH_DEV_*_TASKS.md`
...
| Дата | Изменение |
|------|-----------|
| 2026-03-24 | Блок **«Мини-эпик P2»** — ссылка на `QA_ARCH_P2_CLIENTS_SCHEDULE_FOLLOWUP_2026.md` (follow-up после ревью P2). |
| 2026-03-20 | Первая сборка из актуальных `docs/artifacts/ARCH_DEV_*_TASKS.md` и `ARCH_PERF_ENGINE_L2_DEEP_2026_TASKS.md` (@QA_ARCH). |
