...
> **История схемы:** `alembic/versions/` и git; детальный разбор старых ревизий — только в истории репозитория.
...
Исторический разбор схемы — в git по каталогу `alembic/versions/`.
...
**Фоновый refresh по событию завершения визита (опционально):** переменные **`ERP_AGGREGATE_EVENT_REFRESH_ENABLED`**, **`ERP_AGGREGATE_EVENT_DEBOUNCE_SECONDS`** — см. `.env.example`; **Redis** + Celery `erp_tasks.refresh_clinic_erp_aggregates_window`. Детали NFR — `docs/NONFUNCTIONAL_SCORECARD.md`.

**Выборочная сверка visit_revenue (опционально):** **`ERP_AGGREGATE_PARITY_SAMPLE_ENABLED`** — задача `erp_tasks.run_daily_visit_revenue_parity_sample`; метрики — `docs/METRICS_PROTOCOL.md` / реестр.

Ревизия **`w5perf1idx_fin`:** см. `docs/adr/ADR-005-wave5-replica-reporting-redis-cache.md`, `deploy/grafana/dashboards/`, код миграций.

Ревизия **`r4s5t6u7v8w9`** (CRM/Loyalty): таблицы **`loyalty_groups`**, **`lead_secondary_bookings`**, **`crm_lead_estimated_value_audit`**; nullable **`family_links.group_id`**. Задача Celery **`crm_tasks.reconcile_lead_actual_values`** — по согласованию с OPS.
